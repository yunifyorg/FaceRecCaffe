...
def get_models(name):
    return caffe.Net('./mtcnn/%s.prototxt' % name, './mtcnn/%s.caffemodel' % name, caffe.TEST)
...
def detect_face(im, PNet, RNet, ONet):
    im_bk = im.copy()
...
class MTCNN():
    ...
    def __init__(self):
        # Load models.
        self.PNet = get_models('det1')
        self.RNet = get_models('det2')
        self.ONet = get_models('det3')

    def detect(self, im):
        assert im is not None, 'Image is empty.'
        box, f5pt = detect_face(im, self.PNet, self.RNet, self.ONet)
        return box, np.reshape(f5pt, (-1,5,2), 'F')

    def detect_and_show(self, im):
        bboxes, points = self.detect(im)
        if bboxes.shape[0] == 0:
            return
